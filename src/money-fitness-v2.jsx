...
const sb = {
  ...
  authHeaders: function(token) {
    return Object.assign({}, this.headers, { "Authorization": "Bearer " + token });
  },
  ...
  async insertProfile(token, profile) {
    const r = await fetch(this.url + "/rest/v1/profiles", {
      method: "POST", headers: Object.assign({}, this.authHeaders(token), { "Prefer": "return=representation" }),
                    ...
                    <div key={"fex-"+p.origIdx} style={{background:isC?cc+"15":"#fff",borderRadius:isC?0:14,border:isC?"none":"1.5px solid #E8E4DE",borderBottom:isC?"1px solid "+cc+"30":"none",padding:"12px 14px",marginBottom:isC?0:8}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <button onClick={function(e){e.stopPropagation();loadExHist(p.name);}} style={{background:"none",border:"1px solid #0A1A0F",borderRadius:"50%",width:20,height:20,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0,flexShrink:0}}>
                          <span style={{color:"#0A1A0F",fontSize:10,fontWeight:700,lineHeight:1}}>i</span>
                        </button>
                    ...
                    <div key={"ex"+ex.origIdx} style={{padding:"9px 0",borderBottom:"1px solid #F0EFEC"}}>
                      <div style={{display:"flex",alignItems:"center",gap:8}}>
                        ...
                        <button onClick={function(e){e.stopPropagation();loadExHist(ex.name);}} style={{background:"#F0F5F2",border:"1.5px solid #D0E6D8",borderRadius:"50%",width:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0,flexShrink:0,WebkitTapHighlightColor:"transparent"}}>
                          <span style={{color:"#1B8C4E",fontSize:10,fontWeight:800,lineHeight:1}}>i</span>
                        </button>
...
function AuthFlow({ screen, setScreen, onAuth }) {
  ...
  const inputS = { width: "100%", background: "rgba(255,255,255,0.08)", border: "1.5px solid rgba(255,255,255,0.15)", borderRadius: 14, padding: "14px 16px", color: "#fff", fontSize: 15, outline: "none", boxSizing: "border-box", marginBottom: 4 };
  const errS   = { color: "#FF6B6B", fontSize: 12, marginBottom: 10, paddingLeft: 4 };
  ...
  async function handleSignUp() {
    ...
    if (role === "client" && !coachCode.trim()) e.coachCode = "Enter your coach code";
    setErrors(e);
    ...
    try {
      ...
      var userId = signUpRes.user ? signUpRes.user.id : (signUpRes.id || null);
      var token  = signUpRes.access_token || (signUpRes.session && signUpRes.session.access_token);
      ...
      var profile = {
        ...
        role: role,
        coach_id: coachId,
        coach_code: role === "coach" ? "CMONEY5" : null,
      };
  ...
  if (screen === "welcome") {
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0A1A0F", overflow: "hidden", position: "relative" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 32px" }}>
          ...
          <img src="/mf-logo.png" alt="MF" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
        </div>
          ...
          <button onClick={function() { setScreen("signup"); setRole("client"); }} style={{ width: "100%", padding: "16px", borderRadius: 99, background: "#1B8C4E", border: "none", color: "#fff", fontSize: 16, fontWeight: 800, cursor: "pointer", marginBottom: 12 }}>
            Sign Up
          </button>
  ...
  if (screen === "signup") {
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0A1A0F", overflowY: "auto", padding: "48px 28px 40px" }}>
        <button onClick={function() { setScreen("welcome"); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.5)", fontSize: 14, cursor: "pointer", textAlign: "left", marginBottom: 24, display: "flex", alignItems: "center", gap: 6 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
          Back
        </button>
        <div style={{ color: "#fff", fontSize: 26, fontWeight: 900, marginBottom: 6 }}>Create Account</div>
        <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 14, marginBottom: 28 }}>Join MoneyFitness</div>

        {authError && <div style={{ background: "rgba(255,107,107,0.15)", border: "1px solid #FF6B6B", borderRadius: 10, padding: "12px 14px", color: "#FF6B6B", fontSize: 13, marginBottom: 16 }}>{authError}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5 }}>YOUR NAME</div>
        ...
        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginTop: 8 }}>EMAIL</div>
        <input value={email} onChange={function(e){setEmail(e.target.value);}} placeholder="you@example.com" type="email" style={inputS} />
        {errors.email && <div style={errS}>{errors.email}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginTop: 8 }}>PASSWORD</div>
        <input value={password} onChange={function(e){setPassword(e.target.value);}} placeholder="At least 6 characters" type={showPass ? "text" : "password"} style={inputS} />
        {errors.password && <div style={errS}>{errors.password}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginTop: 8 }}>CONFIRM PASSWORD</div>
        <input value={confirm} onChange={function(e){setConfirm(e.target.value);}} placeholder="Repeat password" type={showPass ? "text" : "password"} style={inputS} />
        {errors.confirm && <div style={errS}>{errors.confirm}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginTop: 8 }}>COACH CODE</div>
        <input value={coachCode} onChange={function(e){setCoachCode(e.target.value.toUpperCase());}} placeholder="Enter code from your coach" style={inputS} />
        {errors.coachCode && <div style={errS}>{errors.coachCode}</div>}

        <button onClick={function(){setShowPass(!showPass);}} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 12, cursor: "pointer", textAlign: "left", marginBottom: 24, marginTop: 4 }}>
          {showPass ? "Hide" : "Show"} password
        </button>

        <button onClick={handleSignUp} disabled={loading} style={{ width: "100%", padding: "16px", borderRadius: 99, background: loading ? "rgba(27,140,78,0.5)" : "#1B8C4E", border: "none", color: "#fff", fontSize: 16, fontWeight: 800, cursor: loading ? "default" : "pointer", marginBottom: 16 }}>
          {loading ? "Creating account..." : "Create Account"}
        </button>

        <div style={{ textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 13 }}>
          Already have an account?{" "}
          <span onClick={function(){setScreen("login");}} style={{ color: "#1B8C4E", fontWeight: 700, cursor: "pointer" }}>Log In</span>
        </div>
  ...
  // Coach sign up screen
  if (screen === "coach-signup") {
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0A1A0F", overflowY: "auto", padding: "48px 28px 40px" }}>
        <button onClick={function() { setScreen("welcome"); }} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.5)", fontSize: 14, cursor: "pointer", textAlign: "left", marginBottom: 24, display: "flex", alignItems: "center", gap: 6 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="15 18 9 12 15 6"/></svg>
        ...
        <div style={{ color: "#fff", fontSize: 26, fontWeight: 900, marginBottom: 6 }}>Coach Sign Up</div>
        <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 14, marginBottom: 28 }}>Create your coach account</div>

        {authError && <div style={{ background: "rgba(255,107,107,0.15)", border: "1px solid #FF6B6B", borderRadius: 10, padding: "12px 14px", color: "#FF6B6B", fontSize: 13, marginBottom: 16 }}>{authError}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5 }}>YOUR NAME</div>
        <input value={name} onChange={function(e){setName(e.target.value);}} placeholder="Your name" style={inputS} />
        {errors.name && <div style={errS}>{errors.name}</div>}
  ...
  if (screen === "login") {
    if (forgotSent) {
      return (
        <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0A1A0F", padding: "48px 28px", alignItems: "center", justifyContent: "center" }}>
          <div style={{ fontSize: 48, marginBottom: 20 }}>📬</div>
          <div style={{ color: "#fff", fontSize: 22, fontWeight: 800, marginBottom: 10, textAlign: "center" }}>Check your inbox</div>
          <div style={{ color: "rgba(255,255,255,0.5)", fontSize: 14, textAlign: "center", marginBottom: 32 }}>We sent a reset link to {forgotEmail}</div>
          <button onClick={function(){ setForgotSent(false); setScreen("login"); }} style={{ background: "none", border: "1.5px solid rgba(255,255,255,0.2)", borderRadius: 99, padding: "12px 28px", color: "#fff", fontSize: 14, cursor: "pointer" }}>
            Back to Login
          </button>
        </div>
      );
    ...
    return (
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0A1A0F", overflowY: "auto", padding: "48px 28px 40px" }}>
        ...
        <div style={{ color: "#fff", fontSize: 26, fontWeight: 900, marginBottom: 6 }}>Welcome back</div>
        <div style={{ color: "rgba(255,255,255,0.45)", fontSize: 14, marginBottom: 28 }}>Log in to MoneyFitness</div>

        {authError && <div style={{ background: "rgba(255,107,107,0.15)", border: "1px solid #FF6B6B", borderRadius: 10, padding: "12px 14px", color: "#FF6B6B", fontSize: 13, marginBottom: 16 }}>{authError}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5 }}>EMAIL</div>
        <input value={email} onChange={function(e){setEmail(e.target.value);}} placeholder="you@example.com" type="email" style={inputS} />
        {errors.email && <div style={errS}>{errors.email}</div>}

        <div style={{ marginBottom: 4, color: "rgba(255,255,255,0.5)", fontSize: 11, fontWeight: 700, letterSpacing: 1.5, marginTop: 8 }}>PASSWORD</div>
        <input value={password} onChange={function(e){setPassword(e.target.value);}} placeholder="Your password" type={showPass ? "text" : "password"} style={inputS} />
        {errors.password && <div style={errS}>{errors.password}</div>}

        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28, marginTop: 4 }}>
          <button onClick={function(){setShowPass(!showPass);}} style={{ background: "none", border: "none", color: "rgba(255,255,255,0.4)", fontSize: 12, cursor: "pointer" }}>
            {showPass ? "Hide" : "Show"} password
          </button>
          <button onClick={function(){ setForgotEmail(email); setForgotSent(false); }} style={{ background: "none", border: "none", color: "#1B8C4E", fontSize: 12, fontWeight: 600, cursor: "pointer" }}
            onFocus={function(){}}
          >
            <span onClick={async function(e){ e.stopPropagation(); if (!forgotEmail && !email.includes("@")) { setAuthError("Enter your email first"); return; } setLoading(true); await sb.resetPassword(forgotEmail || email); setForgotSent(true); setLoading(false); }}>Forgot password?</span>
          </button>
        </div>

        <button onClick={handleLogin} disabled={loading} style={{ width: "100%", padding: "16px", borderRadius: 99, background: loading ? "rgba(27,140,78,0.5)" : "#1B8C4E", border: "none", color: "#fff", fontSize: 16, fontWeight: 800, cursor: loading ? "default" : "pointer", marginBottom: 16 }}>
          {loading ? "Logging in..." : "Log In"}
        </button>

        <div style={{ textAlign: "center", color: "rgba(255,255,255,0.4)", fontSize: 13 }}>
          No account?{" "}
          <span onClick={function(){setScreen("signup");setRole("client");}} style={{ color: "#1B8C4E", fontWeight: 700, cursor: "pointer" }}>Sign Up</span>
        </div>
      </div>
    );
  }
...
function RaceScreen({ activityLogs, raceCollapsed, setRaceCollapsed, plans, setPlans, completions, onToggleCompletion, coachProgram, authUserId, authToken }) {
  ...
  function saveRaceToSupabase(raceObj) {
    if (!authUserId || !authToken) return;
    fetch(SUPABASE_URL + "/rest/v1/profiles?id=eq." + authUserId, {
      method: "PATCH",
      headers: { "apikey": SUPABASE_ANON_KEY, "Authorization": "Bearer " + authToken, "Content-Type": "application/json" },
      body: JSON.stringify({ race_settings: raceObj })
    }).catch(function(){});
  }

  useEffect(function() {
    function onRaceLoaded() {
      try {
...
if (!r.ok) r.text().then(function(t) { console.log("delete entry error:", r.status, t); }); 
  ...
  // Load race settings from Supabase on startup
  useEffect(function() {
  ...
  return (
    <div style={appStyle}>
      ...
      <div style={{ padding: "14px 18px 12px", display: "flex", justifyContent: "space-between", alignItems: "center", background: CARD, borderBottom: "1px solid "+BORDER, flexShrink: 0 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <div style={{ width: 34, height: 34, borderRadius: 8, overflow: "hidden", flexShrink: 0 }}><img src="/mf-logo.png" alt="MF" style={{ width: "100%", height: "100%", objectFit: "cover" }} /></div>
          <div>
